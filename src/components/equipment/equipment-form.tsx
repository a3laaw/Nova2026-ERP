...
export function EquipmentForm({ initialData, onSubmit, loading, isRtl }: Props) {
  ...
  // خريطة الاختصارات السيادية للفئات
  ...
  // محرك التوليد التلقائي للكود بناءً على الاختصار
  useEffect(() => {
    if (!initialData?.id && form.category) {
      const prefix = categoryPrefixes[form.category] || 'EQ';
      
      // نغير الكود آلياً فقط إذا كان فارغاً أو يبدأ بأحد الاختصارات المعروفة (لم يقم المستخدم بتعديله جذرياً)
      ...
      if (isAutoGenerated) {
        const randomSuffix = Math.floor(1000 + Math.random() * 8999); // توليد رقم عشوائي مؤقت لضمان الشكل التلقائي
