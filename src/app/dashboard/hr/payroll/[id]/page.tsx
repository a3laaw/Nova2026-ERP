...
export default function PayrollDetailsPage() {
  ...
  if (!batch) return <div className="p-20 text-center">{t('hr.batchNotFound')}</div>;
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20" dir={dir}>
        ...
        <div className="flex items-center gap-4">
           <div className="text-start">
             <div className="flex items-center gap-3">
                <h1 className="text-3xl font-black font-headline text-slate-900">{t('hr.payrollBatchDetails')}</h1>
                <Badge className={cn(
             ...
             <p className="text-xs font-bold text-muted-foreground mt-1 flex items-center gap-2">
                <ShieldCheck className="h-3 w-3 text-primary" /> {t('hr.cycle')} <span className="font-mono text-slate-800">{batch.month} / {batch.year}</span>
             </p>
           </div>
        </div>

        <div className="flex gap-3">
           <Button variant="outline" className="rounded-xl font-bold h-12 gap-2 border-2"><Printer className="h-4 w-4" /> {t('hr.printList')}</Button>
           {batch.status === 'draft' && (
             <Button onClick={() => handleAction('reviewed')} disabled={processing} className="bg-amber-500 text-white font-black rounded-xl h-12 px-6">
                {processing ? <Loader2 className="animate-spin" /> : t('hr.markReviewed')}
             </Button>
           )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         <Card className="border-0 shadow-lg rounded-[2rem] p-6 text-start bg-white border-b-4 border-emerald-500">
            <p className="text-[10px] font-black text-slate-400 uppercase mb-1">{t('hr.netSalary')}</p>
         ...
         <Card className="border-0 shadow-lg rounded-[2rem] p-6 text-start bg-white border-b-4 border-rose-500">
            <p className="text-[10px] font-black text-slate-400 uppercase mb-1">{t('hr.deductions')}</p>
         ...
         <Card className="border-0 shadow-lg rounded-[2rem] p-6 text-start bg-white border-b-4 border-blue-500">
            <p className="text-[10px] font-black text-slate-400 uppercase mb-1">{t('hr.gross')}</p>
         ...
         <Card className="border-0 shadow-lg rounded-[2rem] p-6 text-start bg-white border-b-4 border-primary">
            <p className="text-[10px] font-black text-slate-400 uppercase mb-1">{t('hr.employeesCount')}</p>
