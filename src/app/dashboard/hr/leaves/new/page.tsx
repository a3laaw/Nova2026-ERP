  ...
  const handleSubmit = async () => {
    ...
    try {
      await leaveService.submitRequest({
        ...
        userName: employee.fullName || globalUser?.fullName || 'User',
        type: form.type,
      ...
      toast({ title: t('common.saved'), description: t('common.saved') });
    ...
    <div className="space-y-8 max-w-5xl mx-auto pb-20 animate-in fade-in duration-700" dir={dir}>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b pb-8 border-slate-200/60">
        <div className="text-start space-y-1">
           ...
           <h1 className="text-3xl font-black font-headline text-slate-900">{t('hr.submitLeaveRequest')}</h1>
        </div>
        ...
        {employee && (
          <div className="bg-white p-4 rounded-2xl shadow-xl ring-1 ring-black/5 flex items-center gap-4">
            ...
            <div className="text-start">
                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">{t('hr.remainingBalance')}</p>
                <h4 className="text-lg font-black text-slate-900">{accruedBalance} <span className="text-[9px] text-muted-foreground font-bold">{t('common.days')}</span></h4>
            </div>
          </div>
        )}
      </div>
             ...
             <Card className="border-0 shadow-xl rounded-[1.5rem] bg-white ring-1 ring-black/5 overflow-hidden text-start">
                ...
                <CardContent className="p-6 space-y-4">
                   <div className="space-y-2">
                      <Label className="text-[9px] font-black uppercase text-slate-400 tracking-widest">{t('hr.searchSelectEmployee')}</Label>
                      ...
                      <Popover open={openEmpPicker} onOpenChange={setOpenEmpPicker}>
                        <PopoverTrigger asChild>
                          <Button variant="outline" className="w-full h-12 rounded-xl border-2 font-bold justify-between px-4 bg-white">
                            <span className="truncate">{employee?.fullName || t('hr.selectEmployee')}</span>
           ...
           <Card className="border-0 shadow-xl rounded-[1.5rem] bg-white ring-1 ring-black/5 overflow-hidden">
              <CardHeader className="bg-slate-50/50 border-b p-6 text-start">
                 <CardTitle className="text-base font-black flex items-center gap-2 text-slate-800">
                    ...
                    {t('hr.fieldCalculation')}
                 </CardTitle>
              </CardHeader>
              <CardContent className="p-6 space-y-6 text-start">
                 <div className="space-y-3">
                    <div className="flex justify-between items-center text-xs font-bold">
                       <span className="text-slate-500">{t('hr.calendarDays')}</span>
                       <span className="font-black text-base">{totalCalendarDays}</span>
                    </div>
                    <div className="flex justify-between items-center text-xs font-bold">
                       <span className="text-slate-500">{t('hr.netBalanceDeduction')}</span>
           ...
           <Card className="border-0 shadow-xl rounded-[1.5rem] bg-white ring-1 ring-black/5 overflow-hidden">
              <CardContent className="p-8 space-y-8">
                 <div className="space-y-1.5 text-start">
                    <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('hr.leaveTypeRequired')}</Label>
                    <Select value={form.type} onValueChange={(v: LeaveType) => setForm({...form, type: v})}>
                        ...
                        <SelectContent className="rounded-xl">
                          <SelectItem value="annual" className="font-bold">{t('hr.annualLeave')}</SelectItem>
                          <SelectItem value="sick" className="font-bold">{t('hr.sickLeave')}</SelectItem>
                          <SelectItem value="emergency" className="font-bold">{t('hr.emergencyLeave')}</SelectItem>
                          <SelectItem value="unpaid" className="font-bold">{t('hr.unpaidLeave')}</SelectItem>
                        </SelectContent>
                    </Select>
                 </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-start">
                    <div className="space-y-1.5">
                       <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('hr.effectiveStartDate')}</Label>
                       <SmartDateInput value={form.startDate} onChange={v => setForm({...form, startDate: v})} />
                    </div>
                    <div className="space-y-1.5">
                       <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('hr.returnToWorkDate')}</Label>
                       <SmartDateInput value={form.endDate} onChange={v => setForm({...form, endDate: v})} />
                    </div>
                 </div>

                 <div className="space-y-1.5 text-start pt-4 border-t">
                    <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('hr.leaveReasonNotes')}</Label>
                    <Textarea 
                       value={form.reason} 
