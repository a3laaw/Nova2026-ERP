            ...
            <div className="flex items-center gap-4">
               <div className="relative w-full max-w-xs">
                  ...
                  <Input 
                    ...
                    className="ps-12 h-10 border-slate-200 focus-visible:ring-primary/10 focus-visible:border-primary transition-all" 
                  />
               </div>
         ...
         <CardContent className="p-0 overflow-x-auto">
            <Table>
               <TableHeader className="bg-muted/10 border-b">
                  <TableRow>
                     <TableHead className="py-5 ps-8 text-start font-black text-slate-500 uppercase text-[10px] tracking-widest">{isRtl ? 'الصنف' : 'Item'}</TableHead>
                     <TableHead className="text-center font-black text-slate-500 uppercase text-[10px] tracking-widest">{isRtl ? 'الكمية' : 'Qty'}</TableHead>
               ...
               <TableBody>
                  {iLoading ? (
                    ...
                    <TableRow><TableCell colSpan={3} className="text-center py-20 italic text-slate-400 font-bold">{isRtl ? 'المخزن فارغ.' : 'Stock is empty.'}</TableCell></TableRow>
                  ) : items?.map(item => (
                    <TableRow key={item.id} className="hover:bg-primary/[0.02] transition-colors border-b-slate-100 group">
                       <TableCell className="py-5 ps-8 font-black text-slate-800">{item.name}</TableCell>
                       ...
                       <TableCell className="pe-8 text-end">
                          <Button variant="ghost" size="icon" className="rounded-xl h-9 w-9 group-hover:bg-primary group-hover:text-white transition-all">
