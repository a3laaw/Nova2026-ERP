      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4 text-start">
          ...
          <div className="text-start">
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">{t('crm.title')}</h1>
            <p className="text-sm font-bold text-muted-foreground italic mt-0.5">{t('crm.description')}</p>
          </div>
        </div>
        ...
        {createAccess.can && (
          <Dialog>
            <DialogTrigger asChild>
              <Button variant="default" className="h-12 px-8 shadow-xl shadow-primary/20 hover:scale-[1.02] transition-all gap-2 border-b-4 border-orange-700">
                <UserPlus className="h-5 w-5" />
                {t('crm.newlead')}
              </Button>
            </DialogTrigger>
            <DialogContent className="rounded-xl border-0 shadow-3xl max-w-lg p-0 overflow-hidden bg-white" dir={dir}>
              ...
              <div className="p-8 space-y-6 text-start">
                ...
                <div className="space-y-2">
                   <Label className="text-xs font-black uppercase text-slate-400">{isRtl ? 'القيمة المتوقعة' : 'Lead Value'}</Label>
        ...
        <div className="p-4 flex flex-col md:flex-row items-center justify-between gap-4 bg-slate-50/30 border-b">
          <div className="relative w-full max-w-md">
            ...
            <Input 
              ...
              className="ps-12 h-11 bg-white border-2 border-slate-100 focus-visible:ring-primary/10 focus-visible:border-primary transition-all font-bold" 
              value={searchTerm} 
              onChange={(e) => setSearchTerm(e.target.value)} 
            />
          </div>
          <Button variant="outline" className="h-11 px-6 border-2 font-black text-xs gap-2">
