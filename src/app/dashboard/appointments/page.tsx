...
import { 
  ...
  Filter, Clock, MapPin, CheckCircle2, XCircle, LayoutGrid
} from "lucide-react";
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500" dir={dir}>
      ...
      <Tabs defaultValue="calendar" className="w-full">
        <TabsList className="bg-white border-2 border-slate-100 p-1 rounded-2xl h-14 gap-2 shadow-sm mb-6">
           <TabsTrigger value="calendar" className="rounded-xl font-black text-xs px-8 data-[state=active]:bg-primary data-[state=active]:text-white transition-all gap-2">
              <LayoutGrid className="h-4 w-4" /> {isRtl ? 'التقويم الزمني' : 'Timeline Grid'}
           </TabsTrigger>
           <TabsTrigger value="list" className="rounded-xl font-black text-xs px-8 data-[state=active]:bg-primary data-[state=active]:text-white transition-all gap-2">
              <CalendarDays className="h-4 w-4" /> {isRtl ? 'قائمة المواعيد' : 'List View'}
           </TabsTrigger>
        </TabsList>

        <TabsContent value="calendar" className="animate-in slide-in-from-bottom-2 duration-500">
           <ArchitecturalAppointmentsView />
        </TabsContent>

        <TabsContent value="list" className="space-y-6 animate-in fade-in">
