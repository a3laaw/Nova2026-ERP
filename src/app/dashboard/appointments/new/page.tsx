...
export default function NewAppointmentPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    clientId: '',
    projectId: '',
    engineerId: globalUser?.employeeId || '', 
    ...
    <div className="space-y-8 max-w-4xl mx-auto pb-20 animate-in slide-in-from-bottom-6 duration-700" dir={dir}>
      <div className="flex items-center gap-4 border-b pb-6">
        ...
        <div className="text-start">
           <h1 className="text-2xl font-black font-headline text-slate-900">{isRtl ? 'جدولة موعد جديد' : 'Schedule New Appointment'}</h1>
          ...
          <CardContent className="p-8 space-y-6 text-start">
             <div className="space-y-1.5">
                ...
                <Input 
                  ...
                  className="h-11 rounded-xl border-2 font-bold bg-slate-50/50" 
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t">
                <div className="space-y-1.5">
                   ...
                   <Select value={formData.clientId} onValueChange={v => {
                      ...
                      setFormData({...formData, clientId: v, projectId: ''});
                   }}>
                      <SelectTrigger className="h-11 rounded-xl border-2 font-bold"><SelectValue placeholder={isRtl ? "اختر من عملائك..." : "Select client..."} /></SelectTrigger>
                      <SelectContent className="rounded-xl">
                         {clients?.map(c => <SelectItem key={c.id} value={c.id!} className="font-bold py-2 border-b last:border-0 border-slate-50">{c.nameAr}</SelectItem>)}
                      </SelectContent>
                   </Select>
                </div>
                <div className="space-y-1.5">
                   <Label className="text-[10px] font-black uppercase text-slate-400">{isRtl ? 'المشروع المرتبط' : 'Project Link'}</Label>
                   <Select disabled={!formData.clientId} value={formData.projectId} onValueChange={v => setFormData({...formData, projectId: v})}>
                      <SelectTrigger className="h-11 rounded-xl border-2 font-bold"><SelectValue placeholder={isRtl ? "اختر من مشاريعك..." : "Select project..."} /></SelectTrigger>
                      <SelectContent className="rounded-xl">
                         {transactions?.filter((t:any) => t.clientId === formData.clientId).map((p:any) => <SelectItem key={p.id} value={p.id!} className="font-bold text-[11px]">{p.subServiceName} - {p.transactionNumber}</SelectItem>)}
                      </SelectContent>
                   </Select>
                </div>
             </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t">
                ...
                <div className="space-y-1.5">
                   ...
                   <Input type="time" value={formData.time} onChange={e => setFormData({...formData, time: e.target.value})} className="h-11 rounded-xl border-2 font-black text-lg" />
                </div>
             </div>

             <div className="space-y-1.5 pt-6 border-t">
                <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{isRtl ? 'ملاحظات' : 'Notes'}</Label>
                <Textarea value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="min-h-[100px] rounded-xl border-2 p-4 text-xs font-bold bg-slate-50/30" placeholder="..." />
             </div>

             <Button 
               type="submit" 
               disabled={loading || !formData.clientId}
               className="w-full h-14 rounded-2xl bg-primary text-white font-black text-lg shadow-xl shadow-primary/20 hover:scale-[1.02] transition-all gap-4 mt-4"
             >
