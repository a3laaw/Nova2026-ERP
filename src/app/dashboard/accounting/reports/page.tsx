    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 text-start max-w-[1600px] mx-auto" dir={dir}>
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b pb-6">
        <div className="text-start space-y-1">
           ...
           <h1 className="text-4xl font-black font-headline text-slate-800">{isRtl ? 'تحليل مراكز التكلفة والربحية' : 'Cost & Profit Center Analysis'}</h1>
      ...
      <Card className="border-0 shadow-2xl rounded-[2rem] bg-white ring-1 ring-black/5 p-8 overflow-visible print:hidden">
         <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
               ...
               <SearchableDropdown
                 options={[
                   ...
                   ...(allTransactions || []).map(t => ({ 
                      id: t.id, 
                      name: t.clientName, 
                      subText: `${t.subServiceName} (#${t.transactionNumber})` 
                    }))
                 ]}
            ...
            <div className="space-y-2 text-start">
               ...
               <SearchableDropdown
                 options={[
                   { id: 'all', name: isRtl ? '--- كافة مراكز التكلفة ---' : '--- All Cost Centers ---' },
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                   ...
                   <Card className="border-0 shadow-xl rounded-[2rem] bg-white p-8 border-b-8 border-[#2563EB] ring-1 ring-black/5 text-start">
                      ...
                      <h3 className="text-3xl font-black text-[#2563EB]">{stats.totalBudget.toLocaleString()} <span className="text-xs">KWD</span></h3>
                   </Card>
                </div>
                         ...
                         <tbody className="divide-y divide-slate-100">
                            {projectsData.map((p, i) => (
                               <tr key={i} className="hover:bg-slate-50/50 transition-colors cursor-pointer" onClick={() => setFilters(prev => ({ ...prev, projectId: p.projectId }))}>
                                  ...
                                  <td className="p-6 text-end font-mono font-bold text-slate-700">{p.totalRevenue.toLocaleString()}</td>
               ...
               <Card className="border-0 shadow-xl rounded-[2.5rem] bg-white overflow-hidden ring-1 ring-black/5">
                  <CardHeader className="bg-slate-50/50 border-b p-8">
                     <CardTitle className="text-lg font-black flex items-center gap-3 text-start text-slate-800">
                        <UserCheck className="h-6 w-6 text-[#2563EB]" /> {isRtl ? 'جدوى القوى العاملة (Labor ROI)' : 'Labor Efficiency (ROI)'}
                     </CardTitle>
                  </CardHeader>
