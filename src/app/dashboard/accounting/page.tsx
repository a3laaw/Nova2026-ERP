...
export default function AccountingPage() {
  ...
  return (
    <div className="space-y-8" dir="rtl">
      <div>
        <h1 className="text-3xl font-black font-headline text-right">الإدارة المالية والتسوية</h1>
        <p className="text-muted-foreground mt-1 text-right">المطابقة الذكية للأرصدة وكشوف الحسابات بالذكاء الاصطناعي</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <CardHeader className="bg-primary/5 border-b p-6">
            <div className="flex items-center justify-between flex-row-reverse">
              <div className="text-right">
                <CardTitle className="text-lg font-bold">كشف الحساب البنكي الحالي</CardTitle>
                <CardDescription>العمليات الأخيرة المستوردة من بنك الكويت الوطني</CardDescription>
              </div>
              <Badge variant="outline" className="bg-white border-primary/20 text-primary">نشط</Badge>
            </div>
          </CardHeader>
              ...
              <TableBody>
                {initialBankEntries.map((entry) => (
                  <TableRow key={entry.id}>
                    ...
                    <TableCell className="font-mono text-left font-bold text-xs">{entry.amount.toLocaleString()} د.ك</TableCell>
        ...
        <Card className="border-0 shadow-lg rounded-2xl bg-white overflow-hidden">
          <CardHeader className="bg-secondary/40 border-b p-6">
            <div className="flex items-center justify-between flex-row-reverse">
              <div className="text-right">
                <CardTitle className="text-lg font-bold">دفتر الأستاذ العام (ERP)</CardTitle>
                <CardDescription>القيود المحاسبية المرحلة في نظام نوفا المالي</CardDescription>
              </div>
              <Badge variant="outline" className="bg-white">المطابقة المبدئية</Badge>
            </div>
          </CardHeader>
          <CardContent className="p-6">
            <Table>
              ...
              <TableBody>
                {initialLedgerEntries.map((entry) => (
                  <TableRow key={entry.id}>
                    <TableCell className="font-mono text-xs">{entry.date}</TableCell>
                    <TableCell className="font-bold text-xs text-primary">{entry.accountName}</TableCell>
      ...
      <div className="flex justify-center">
        <Button
          ...
          className="bg-primary text-white font-bold rounded-2xl px-10 py-6 text-lg shadow-xl shadow-primary/20 hover:scale-[1.02] transition-transform"
        >
          {reconciling ? (
            <>
              <RefreshCw className="ml-2 h-5 w-5 animate-spin" />
              جاري مطابقة القيود وتحليل الفروقات مالياً...
            </>
          ) : (
            <>
              <Sparkles className="ml-2 h-5 w-5" />
              تشغيل نظام المطابقة والتسوية الذكي (AI)
            </>
          )}
        </Button>
      </div>
      ...
      {reconResult && (
        <Card className="border-2 border-primary/20 shadow-2xl rounded-3xl bg-white overflow-hidden animate-in slide-in-from-bottom-4 duration-500">
          <CardHeader className="bg-primary/5 p-8 border-b">
            <CardTitle className="font-headline font-bold text-xl flex items-center gap-2 flex-row-reverse text-right">
              <Calculator className="h-6 w-6 text-primary" />
              تقرير التسوية والمطابقة الذكي المستنتج
            </CardTitle>
            <CardDescription className="text-right">تقرير فوري صادر عن وكيل الذكاء الاصطناعي المالي لنوفا</CardDescription>
          </CardHeader>
          <CardContent className="p-8 space-y-6 text-right">
            ...
            <div className="space-y-4">
              <h4 className="font-bold text-lg">المطابقات المقترحة وثقة الذكاء الاصطناعي</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
              <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 space-y-2">
                <div className="flex items-center gap-2 flex-row-reverse text-amber-800 font-bold">
                  <AlertTriangle className="h-5 w-5" />
                  <h5>حركات بنكية معلقة يدوياً</h5>
                </div>
                <p className="text-xs text-amber-700">عدد العمليات غير المطابقة: {reconResult.unmatchedBankEntries?.length || 0}</p>
              </div>
              <div className="p-4 rounded-2xl bg-blue-50 border border-blue-200 space-y-2">
                <div className="flex items-center gap-2 flex-row-reverse text-blue-800 font-bold">
