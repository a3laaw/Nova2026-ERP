...
export default function FieldVisitDetailsPage() {
  ...
  return (
    <div className="space-y-6 w-full max-w-[1600px] mx-auto pb-20 animate-in fade-in duration-500 text-start" dir={dir}>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 pb-4 print:hidden px-4 pt-4 text-start">
        ...
        <div className="flex gap-2">
           ...
           {!isReviewing ? (
             <Button onClick={() => setIsReviewing(true)} variant="outline" size="sm" className="h-11 px-6 rounded-xl font-black border-2 bg-white gap-2 text-primary">
               <MessageSquare className="h-4 w-4" /> {t('common.response')}
             </Button>
           ) : (
             <div className="flex gap-2">
           ...
           <Button variant="outline" size="sm" onClick={() => window.print()} className="h-11 w-11 rounded-xl border-2 bg-white shadow-sm">
      ...
      <PrintWrapper title={t('construction.fieldProgressStatement')}>
         <div className="space-y-12 text-start">
            ...
            {/* Sequential Content Flow (Replacing Tabs) */}
            
            {/* 1. Technical Progress Section */}
            <div className="space-y-6">
               <div className="flex items-center gap-3 px-2 border-b-2 border-slate-100 pb-2">
                  <Hammer className="h-6 w-6 text-primary" />
                  <h3 className="text-xl font-black font-headline text-slate-900">{t('construction.technicalProgress')}</h3>
               </div>
               
               <div className="border-2 border-slate-100 rounded-[2.5rem] bg-white overflow-hidden shadow-xl ring-1 ring-black/[0.02]">
                  <Table>
                     <TableHeader className="bg-slate-50/80 border-b-2">
                        <TableRow className="hover:bg-slate-50 border-0">
                           ...
                           <TableHead className="text-center w-[120px] text-primary font-black uppercase text-[10px] tracking-widest">{t('common.quantity')}</TableHead>
            ...
            <div className="space-y-6 pt-10">
               <div className="flex items-center gap-3 px-2 border-b-2 border-slate-100 pb-2">
                  ...
                  <h3 className="text-xl font-black font-headline text-slate-900">{isRtl ? 'الموارد الموثقة (عمالة ومعدات)' : 'Logged Resources'}</h3>
               </div>

               <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  {/* Labor */}
                  <div className="space-y-4">
                     <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest ps-4">{isRtl ? 'الموارد البشرية' : 'Human Resources'}</h4>
                     <div className="grid grid-cols-1 gap-3">
                        {visit.laborDetails?.map((l: any, i: number) => (
                           <div key={i} className="flex justify-between items-center p-5 bg-slate-50/50 border-2 border-white rounded-[1.5rem] shadow-sm">
                              <div className="flex items-center gap-4 text-start">
                                 ...
                                 <div>
                                    <p className="font-black text-slate-800 text-sm">{l.trade}</p>
                                    <p className="text-[9px] font-bold text-slate-400 uppercase">{l.hours} HRS LOGGED</p>
                                 </div>
                              </div>
                              <Badge variant="outline" className="bg-white text-slate-900 border-2 font-black h-7 px-4 rounded-lg">{l.count} STAFF</Badge>
                           </div>
                        ))}
                        {!visit.laborDetails?.length && <p className="text-xs font-bold text-slate-300 italic p-8 text-center border-2 border-dashed rounded-3xl">لا توجد سجلات عمالة.</p>}
                     </div>
                  </div>

                  {/* Equipment */}
                  <div className="space-y-4">
                     <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest ps-4">{isRtl ? 'المعدات والآليات' : 'Heavy Equipment'}</h4>
                     <div className="grid grid-cols-1 gap-3">
                           ...
                           <div key={i} className="flex justify-between items-center p-5 bg-blue-50/30 border-2 border-white rounded-[1.5rem] shadow-sm">
                              <div className="flex items-center gap-4 text-start">
                                 ...
                                 <div>
                                    ...
                                    <p className="text-[9px] font-bold text-blue-400 uppercase">TRACKED USAGE</p>
                                 </div>
                              </div>
                        ...
                        {!visit.equipmentUsed?.length && <p className="text-xs font-bold text-slate-300 italic p-8 text-center border-2 border-dashed rounded-3xl">لا توجد سجلات معدات.</p>}
                     </div>
                  </div>
               </div>
            </div>
            ...
            <div className="p-10 bg-white border-2 border-slate-100 rounded-[3rem] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8 border-t-8 border-primary text-start">
               ...
               <div className="flex gap-10 items-center">
                  <div className="text-center md:text-end space-y-1">
                     <p className="text-[8px] font-black text-slate-500 uppercase">{isRtl ? 'بوابة التحقق' : 'Validation'}</p>
                     <div className="flex items-center gap-2">
                        <CheckCircle2 className={cn("h-4 w-4", visit.isVerified ? "text-emerald-500" : "text-slate-200")} />
                        <span className="text-[10px] font-black uppercase">{visit.isVerified ? 'VERIFIED' : 'PENDING'}</span>
                     </div>
                  </div>
                  <div className="text-center md:text-end space-y-1">
