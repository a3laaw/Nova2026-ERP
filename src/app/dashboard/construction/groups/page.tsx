    ...
    <div className="space-y-6 animate-in fade-in duration-700" dir={dir}>
          ...
          <DialogContent className="rounded-[2.5rem] p-0 overflow-hidden border-0 shadow-3xl bg-white" dir={dir}>
             ...
             <div className="p-8 space-y-6 text-start max-h-[65vh] overflow-y-auto scrollbar-hide bg-white">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   ...
                   <div className="space-y-2">
                      <Label className="text-xs font-black uppercase text-slate-400">{isRtl ? 'تصنيف المجموعة' : 'Category'}</Label>
                      <Select value={form.category} onValueChange={v => setForm({...form, category: v})}>
                         <SelectTrigger className="h-11 border-2 font-bold rounded-xl"><SelectValue /></SelectTrigger>
                ...
                <div className="space-y-4 pt-4 border-t">
                   <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                      <div className="space-y-1">
                         <Label className="text-xs font-black uppercase text-primary flex items-center gap-2">
                           <Building2 className="h-4 w-4" /> {isRtl ? 'تصفية حسب القسم' : 'Filter by Department'}
                         </Label>
                         <p className="text-[9px] text-slate-400 font-bold uppercase">{isRtl ? 'اختر القسم لحصر الموظفين' : 'Select dept to filter members'}</p>
                      </div>
                      <Select value={filterDeptId} onValueChange={setFilterDeptId}>
                         <SelectTrigger className="h-10 w-full md:w-64 border-2 font-black rounded-xl bg-slate-50/50 shadow-inner">
                            <SelectValue />
                         </SelectTrigger>
                         <SelectContent className="rounded-xl border-0 shadow-2xl z-[150]">
                            <SelectItem value="all" className="font-bold">{isRtl ? 'عرض كافة الأقسام' : 'All Departments'}</SelectItem>
                   ...
                   <div className="space-y-3">
                      <Label className="text-xs font-black uppercase text-slate-400 tracking-widest">{isRtl ? 'أعضاء الطاقم المختارين' : 'Crew Members Selection'}</Label>
                         ...
                         <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            {filteredEmployeesForSelection.length === 0 && (
                               <div className="col-span-full py-10 text-center opacity-30">
                                  ...
                                  <p className="text-xs font-bold text-slate-400 mt-2">{isRtl ? 'لا يوجد موظفين في هذا القسم.' : 'No employees in this department.'}</p>
                               </div>
                            )}
                         </div>
                      </ScrollArea>
             ...
             <DialogFooter className="p-8 bg-slate-50 border-t">
                <Button onClick={handleSave} disabled={loading || !form.name} className="w-full h-16 rounded-2xl bg-primary text-white font-black text-xl shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all border-b-8 border-orange-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         {[
           ...
           { label: isRtl ? 'العمالة الموزعة' : 'Deployed Force', val: groups?.reduce((acc, g) => acc + (g.memberCount || 0), 0) || 0, icon: HardHat, color: 'text-blue-600', bg: 'bg-blue-50' },
           { label: isRtl ? 'التصنيفات الفنية' : 'Categories', val: new Set(groups?.map(g => g.category)).size || 0, icon: LayoutGrid, color: 'text-emerald-600', bg: 'bg-emerald-50' },
         ].map((stat, i) => (
      ...
      <Card className="border-0 shadow-xl rounded-[2.5rem] bg-white overflow-hidden ring-1 ring-black/5">
         <CardHeader className="bg-slate-50/50 border-b p-6 flex flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
               <Button variant="outline" size="sm" className="h-10 rounded-xl border-2 font-bold px-5 bg-white"><Filter className="h-4 w-4 me-2" /> {isRtl ? 'تصفية المجموعات' : 'Filters'}</Button>
            </div>
         </CardHeader>
                    ...
                    <TableRow key={group.id} className="hover:bg-primary/[0.02] transition-colors border-b-slate-100 group cursor-pointer" onClick={() => { setForm(group); setIsAddOpen(true); }}>
                       <TableCell className="py-5 ps-8 text-start">
                          <div className="flex flex-col">
                             ...
                             <Badge variant="secondary" className="w-fit text-[8px] font-black uppercase bg-primary/5 text-primary border-0 mt-1.5 px-2">{group.category}</Badge>
                          </div>
                       </TableCell>
