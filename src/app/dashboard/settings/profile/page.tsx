...
import { 
  UserCog, ShieldCheck, Loader2, Save, UserCircle, Camera, Lock
} from "lucide-react";
...
export default function ProfilePage() {
  ...
  const [formData, setFormData] = useState({ fullName: '', username: '', photoUrl: '' });
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 1024 * 1024) {
      toast({ variant: "destructive", title: t('common.error'), description: isRtl ? "حجم الصورة كبير جداً" : "Image too large" });
      return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateDoc(tenantUserRef, { displayName: formData.fullName, username: formData.username, photoUrl: formData.photoUrl, updatedAt: serverTimestamp() });
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto pb-20" dir={dir}>
      <div className="text-start">
        <h1 className="text-4xl font-black font-headline flex items-center gap-3 text-slate-900"><UserCog className="h-10 w-10 text-primary" /> {t('profile')}</h1>
        <p className="text-muted-foreground mt-1 text-sm font-bold opacity-80 italic text-start">{t('settings.profile.desc')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <Card className="border-0 shadow-xl rounded-[2.5rem] bg-white overflow-hidden ring-1 ring-black/5">
             ...
             <CardContent className="p-8 -mt-12 text-center flex flex-col items-center">
                <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                   <Avatar className="h-28 w-28 rounded-[2rem] border-4 border-white shadow-2xl transition-transform group-hover:scale-105">
                      <AvatarImage src={formData.photoUrl} className="object-cover" />
                      <AvatarFallback className="bg-primary/10 text-primary font-black text-2xl uppercase">{formData.fullName?.charAt(0)}</AvatarFallback>
                   </Avatar>
                   <div className="absolute inset-0 bg-black/40 rounded-[2rem] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><Camera className="h-8 w-8 text-white" /></div>
                   <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/*" />
                </div>
                <div className="mt-6 space-y-2">
                   <h3 className="text-xl font-black font-headline text-slate-900 leading-tight">{formData.fullName}</h3>
                   <Badge className="bg-primary/5 text-primary font-black border-primary/20 uppercase text-[9px] tracking-widest px-4 py-1 rounded-full">{globalUser?.roleCode || 'USER'}</Badge>
                </div>
             </CardContent>
          </Card>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
           <Card className="border-0 shadow-xl rounded-[2.5rem] bg-white overflow-hidden ring-1 ring-black/5 text-start">
              <CardHeader className="bg-slate-50/50 border-b p-8 text-start"><CardTitle className="text-lg font-black font-headline flex items-center gap-2 text-slate-800"><UserCircle className="h-5 w-5 text-primary" /> {t('personalInfo')}</CardTitle></CardHeader>
           ...
           <div className="flex justify-end pt-4"><Button onClick={handleSave} disabled={saving} className="h-16 rounded-2xl px-16 bg-primary text-white font-black text-xl shadow-xl shadow-primary/20 hover:scale-[1.02] transition-all gap-4 border-b-8 border-orange-700">{saving ? <Loader2 className="animate-spin h-6 w-6" /> : <Save className="h-6 w-6" />}{t('saveChanges')}</Button></div>
        </div>
      </div>
    </div>
  );
