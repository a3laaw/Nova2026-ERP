  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setActivityForm({ name: '', nameEn: '' })} className="btn-nova-primary h-12 rounded-xl shadow-lg"><Plus className="me-2 h-4 w-4" /> {isRtl ? 'نشاط جديد' : 'New Activity'}</Button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* العمود الأول: الأنشطة */}
                ...
                <div key={sub.id} className={cn("p-5 border-b flex items-center justify-between group transition-all", selectedSub?.id === sub.id ? 'bg-emerald-50/50 border-s-4 border-s-emerald-500' : 'hover:bg-muted/30')}>
                  ...
                  <div className="flex items-center gap-1 z-20">
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" disabled={loadingAction === `delete_${sub.id}`} onClick={() => setDeletingContext({ id: sub.id!, type: 'sub' })}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {/* حوارات الإدخال (Activity, Service, Sub) */}
      {activityForm && (
        ...
        <AlertDialogContent className="rounded-[2rem] p-8" dir={dir}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-start font-bold">
              {isRtl ? 'سيؤدي هذا لحذف كافة العناصر والخدمات التابعة لهذا المسار بشكل نهائي من قاعدة البيانات.' : 'This will permanently delete all sub-items and services linked to this path.'}
            </AlertDialogDescription>
          </AlertDialogHeader>
