...
import { 
  ...
  Trash2, Edit3, Folder, Hammer, 
  ChevronRight, ChevronDown, Save, 
  Settings2, Workflow, RotateCcw, 
  MapPin, AlertTriangle, CheckCircle2, X, 
  ShieldAlert, Database, Sparkles
} from "lucide-react";
...
export default function BOQNodesPage() {
  ...
  }, [db, companyId, effectiveContext?.subServiceId, editingNode?.isExecutable, effectiveContext?.activityTypeId, effectiveContext?.serviceId]);
      ...
      <div key={node.id} className="space-y-1">
        ...
        >
          <div className="flex items-center gap-3">
             <div onClick={() => setExpandedNodes(prev => prev.includes(node.id) ? prev.filter(n => n !== node.id) : [...prev, node.id])} className="cursor-pointer text-slate-400 hover:text-primary transition-colors">
               ...
               <div className="p-6 bg-slate-50 rounded-2xl border-2 border-white shadow-inner space-y-6">
                  ...
                  {editingNode?.parentId ? (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 animate-in fade-in">
                       ...
                       <div className="space-y-1">
                          <Label className="text-[9px] font-black uppercase text-slate-400">{isRtl ? 'المسار الفني' : 'Path'}</Label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                       <div className="space-y-1.5">
                          <Label className="text-[10px] font-black">{isRtl ? 'النشاط' : 'Activity'}</Label>
                       ...
                       <div className="space-y-1.5">
                          <Label className="text-[10px] font-black">{isRtl ? 'الخدمة' : 'Service'}</Label>
                       ...
                       <div className="space-y-1.5">
                          <Label className="text-[10px] font-black">{isRtl ? 'المسار' : 'Path'}</Label>
                          ...
                          {effectiveContext?.subServiceId ? (
                                ...
                                <div className="rounded-[2rem] border-2 border-primary/20 bg-white p-4 shadow-2xl space-y-4 animate-in fade-in-0 zoom-in-95 duration-200">
                                  <div className="relative">
                                    ...
                                    <Input
                                      ...
                                      placeholder={tSafe('ref.node.searchStage', 'ابحث باسم المرحلة (مثلاً: حفر، قواعد)...', 'Search Stage')}
                                    ...
                                    <div className="p-3 space-y-1.5">
                                      {loadingStages ? (
                                        <div className="py-20 text-center flex flex-col items-center gap-3">
                                          ...
                                          <p className="text-[10px] font-black text-slate-400 uppercase">{tSafe('ref.node.fetchingStages', 'جاري جلب مراحل المسار الفني...', 'Fetching...')}</p>
                                        </div>
                                      ) : availableStages.length === 0 ? (
                                        <div className="py-16 text-center text-xs font-bold text-slate-400 italic">{tSafe('ref.node.noStages', 'لم يتم العور على مراحل في هذا المسار الفني', 'No Stages')}</div>
                                      ) : (
                                            ...
                                            >
                                              <div className="flex items-center gap-4 min-w-0">
                                                ...
                                                <div className="text-start min-w-0">
                                                  <p className={cn("font-black text-xs truncate", isChecked ? "text-slate-900" : "text-slate-500")}>{isRtl ? stage.name : stage.nameEn}</p>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                       {editingNode.technicalStageIds.map(id => {
                                          ...
                                          return (
                                            ...
                                            >
                                               <span className="text-[11px] font-black text-slate-700 truncate">{isRtl ? stage?.name : stage?.nameEn}</span>
                            ...
                            <div className="flex items-center gap-4 text-rose-50 p-6 bg-rose-50/50 rounded-[2rem] border-2 border-rose-100 ring-4 ring-rose-50/20 text-start">
                               ...
                               <div className="text-start space-y-1">
                                  ...
                                  <p className="text-[10px] font-bold text-rose-600/80 leading-relaxed">{tSafe('ref.node.unknownPathDesc', 'هذا البند غير تابع لمسار فني (SubService). يرجى ربط العقدة الأب بمسار محدد أولاً لتتمكن من تعيين مراحل التنفيذ.', 'Link parent first.')}</p>
                               </div>
                            </div>
                          )}
                       </div>
      ...
      <AlertDialog open={!!deletingId} onOpenChange={open => !open && setDeletingId(null)}>
        <AlertDialogContent className="rounded-[2.5rem] p-10 border-0 shadow-3xl bg-white" dir={dir}>
          <AlertDialogHeader>
             ...
             <AlertDialogDescription className="text-start font-bold text-slate-400 mt-4 text-lg leading-relaxed">{tSafe('ref.node.confirmDeleteDesc', 'هل أنت متأكد؟ سيتم إزالة البند نهائياً من القاموس وكافة ارتباطاته الميدانية بالدراسات والقوالب نهائياً. لا يمكن التراجع عن هذا الإجراء.', 'Confirm deletion')}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-12 gap-4 flex flex-row">
            ...
            <AlertDialogAction onClick={handleFinalDelete} className="flex-[2] h-16 rounded-2xl font-black bg-rose-600 hover:bg-rose-700 text-white shadow-xl shadow-rose-200">{t('common.delete')}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
