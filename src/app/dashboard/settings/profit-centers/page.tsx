    ...
    <div className="space-y-6 text-start" dir={dir}>
      <header className="flex justify-between items-center">
        <h1 className="text-2xl font-black font-headline flex items-center gap-3">
          ...
          {tSafe('inline.profit.centers', 'مراكز الربحية', 'Profit Centers')}
        </h1>
        <Button onClick={() => setEditingItem({ code: '', name: '', isActive: true })} className="rounded-xl shadow-lg bg-emerald-600 hover:bg-emerald-700">
          ...
          <Table>
            <TableHeader className="bg-muted/30">
              <TableRow>
                ...
                <TableHead className="text-start">{isRtl ? 'المشروع المرتبط' : 'Linked Project'}</TableHead>
              ...
              {loading ? (
                ...
                filtered.map(item => (
                  <TableRow key={item.id} className="hover:bg-slate-50 transition-colors border-b-slate-100">
                    <TableCell className="py-5 ps-8 font-mono font-black text-emerald-600">{item.code}</TableCell>
      ...
      <Dialog open={!!editingItem} onOpenChange={open => !open && setEditingItem(null)}>
        <DialogContent className="rounded-xl p-0 overflow-hidden max-w-lg border-0 shadow-3xl bg-white">
           <div className="bg-emerald-50 p-8 text-emerald-900 text-start border-b">
           ...
           <div className="p-8 space-y-6 text-start bg-white">
              ...
              <div className="space-y-1.5">
                 ...
                 <Select value={editingItem?.projectId || 'NONE'} onValueChange={v => setEditingItem({...editingItem!, projectId: v === 'NONE' ? '' : v})}>
           ...
           <DialogFooter className="p-8 bg-slate-50 border-t">
              <Button onClick={handleSave} disabled={loadingAction === 'save' || !editingItem?.code || !editingItem?.name} className="w-full h-14 rounded-xl font-black bg-emerald-600 hover:bg-emerald-700">
