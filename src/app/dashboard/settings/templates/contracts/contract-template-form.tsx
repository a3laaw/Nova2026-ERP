...
import { 
  ...
  Gavel, Calculator, Info, DollarSign, ShieldCheck,
  Zap, AlertTriangle
} from "lucide-react";
            ...
            <div className="space-y-6">
               <div className="flex justify-between items-center px-6">
                  <h3 className="text-2xl font-black font-headline text-slate-800 flex items-center gap-3">
                     ...
                     {isRtl ? 'هيكلة دفعات التعاقد' : 'Contract Milestones'}
                  </h3>
               </div>
               ...
               <div className="p-10 bg-emerald-50/40 rounded-[3rem] border-2 border-emerald-100/50 text-start relative overflow-hidden group shadow-sm transition-all hover:bg-emerald-50">
                  ...
                  <div className="max-w-md mx-auto space-y-3 relative z-10 text-center">
                     <Label className="text-[11px] font-black uppercase text-emerald-600 tracking-widest flex items-center justify-center gap-2">
                        {isRtl ? 'إجمالي قيمة العقد التقديرية (KWD)' : 'Total Contract Value (KWD)'}
                     </Label>
                     <Input 
                        type="number" 
                    ...
                    <Card key={idx} className="border-0 shadow-lg rounded-[2.5rem] bg-white ring-1 ring-black/5 overflow-hidden group hover:ring-2 hover:ring-primary/10 transition-all">
                       <CardContent className="p-8 grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
                          ...
                          <div className="md:col-span-4 space-y-2 text-start">
                             <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{isRtl ? 'مسمى الدفعة' : 'Milestone Name'}</Label>
                          ...
                          <div className="md:col-span-2 space-y-2 text-start">
                             ...
                             <Input type="number" value={m.percentage || 0} onChange={e => updateMilestone(idx, 'percentage', Number(e.target.value))} className="h-11 border-2 font-black text-emerald-600 rounded-xl text-center" onWheel={(e) => (e.target as HTMLInputElement).blur()} />
                          </div>
                          <div className="md:col-span-4 space-y-2 text-start">
                             <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{isRtl ? 'الربط بالمرحلة الفنية' : 'Link Stage'}</Label>
                             <Select value={m.technicalStageId || ''} onValueChange={v => updateMilestone(idx, 'technicalStageId', v)}>
                                <SelectTrigger className="h-11 font-bold border-2 rounded-xl bg-slate-50/30"><SelectValue placeholder="..." /></SelectTrigger>
                                <SelectContent className="rounded-xl">
                                   <SelectItem value="MANUAL" className="font-black text-xs text-primary">{isRtl ? 'تفعيل يدوي' : 'Manual Trigger'}</SelectItem>
                                   {stages?.map(s => <SelectItem key={s.id} value={s.id!} className="font-bold text-xs">{isRtl ? s.name : s.nameEn}</SelectItem>)}
                                </SelectContent>
                             </Select>
                          </div>
                          <div className="md:col-span-1 flex justify-end">
                             <Button variant="ghost" size="icon" onClick={() => removeMilestone(idx)} className="h-11 w-11 rounded-xl text-rose-300 hover:text-rose-600"><Trash2 className="h-5 w-5" /></Button>
                          </div>
                       </CardContent>
                    </Card>
                  ))}
                  <Button variant="outline" onClick={addMilestone} className="w-full h-16 rounded-[2.5rem] border-2 border-dashed border-primary/20 text-primary font-black gap-3 hover:bg-primary/5 transition-all shadow-sm"><Plus className="h-6 w-6" /> {t('addMilestone')}</Button>
               </div>
               ...
               <div className={cn(
                 ...
                 isMathValid ? "bg-emerald-50 border-emerald-200 text-emerald-800" : "bg-rose-50 border-rose-200 text-rose-800"
                  ...
                  <div className="text-center bg-white p-6 rounded-[2.5rem] shadow-xl min-w-[220px]">
                  ...
                  <div className="text-start md:text-end mt-6 md:mt-0 space-y-2">
                     <p className="font-black text-2xl font-headline leading-tight">{t('totalQuoteShare')}</p>
                     <p className="text-[10px] font-bold opacity-60 uppercase tracking-[0.15em]">{isMathValid ? 'BALANCED' : 'INVALID TOTAL'}</p>
                  </div>
               </div>
            </div>
         </div>
         ...
         <div className="lg:col-span-4 space-y-6">
            <Card className="border-0 shadow-xl rounded-[2.5rem] bg-white ring-1 ring-black/5 overflow-hidden text-start">
               <CardHeader className="bg-slate-900 text-white p-6">
                  <CardTitle className="text-sm font-black flex items-center gap-3"><Gavel className="h-5 w-5 text-primary" /> {isRtl ? 'الدستور القانوني للعقد' : 'Legal Constitution'}</CardTitle>
               </CardHeader>
               <CardContent className="p-8 space-y-6">
                  <div className="space-y-3">
                     <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{t('introText')}</Label>
                     <Textarea value={formData.introText || ''} onChange={e => setFormData({...formData, introText: e.target.value})} className="min-h-[100px] rounded-2xl bg-slate-50/50 p-4 border-2 text-xs font-bold" />
                  </div>
                  <div className="space-y-3">
                     <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{isRtl ? 'البنود والالتزامات' : 'Main Clauses'}</Label>
                     <Textarea value={formData.legalText || ''} onChange={e => setFormData({...formData, legalText: e.target.value})} className="min-h-[350px] rounded-2xl bg-slate-50/50 p-6 border-2 text-xs font-bold leading-relaxed shadow-inner" />
                  </div>
                  <div className="p-5 rounded-[2rem] bg-blue-50/50 border-2 border-white shadow-inner flex items-start gap-4">
                     <Info className="h-5 w-5 text-blue-600 mt-1 shrink-0" />
                     <p className="text-[9px] text-blue-700 font-bold leading-relaxed">
                        {isRtl ? 'سيقوم النظام آلياً باستبدال المتغيرات عند إصدار العقد الفعلي من المعاملة.' : 'Variables will be auto-replaced upon contract instantiation.'}
                     </p>
                  </div>
               </CardContent>
            </Card>
