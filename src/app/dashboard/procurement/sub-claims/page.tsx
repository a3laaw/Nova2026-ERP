...
export default function GlobalSubClaimsPage() {
  ...
  const subIpcsQuery = useMemo(() => 
    companyId && db ? query(collection(db, paths.subIpcs(companyId)), orderBy('createdAt', 'desc')) : null, 
  [db, companyId]);

  const { data: ipcs, loading } = useCollection<any>(subIpcsQuery);

  const filtered = (ipcs || []).filter(i => 
    i.subcontractorName?.toLowerCase().includes(searchTerm.toLowerCase()) || 
                ...
                <TableRow key={ipc.id} className="hover:bg-slate-50 transition-colors border-b-slate-100 group cursor-pointer" onClick={() => router.push(`/dashboard/clients/${ipc.clientId || 'none'}/transactions/${ipc.transactionId}?tab=documents`)}>
                   <TableCell className="py-6 ps-10">
                      <div className="flex items-center gap-4 text-start">
                         <div className="h-11 w-11 rounded-xl bg-slate-900 text-white flex items-center justify-center font-black shadow-inner">
                            SIP
                         </div>
                   ...
                   <TableCell className="text-center">
                      <Badge className={cn(
                        ...
                        ipc.status === 'paid' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                      )}>
                         {ipc.status === 'paid' ? <CheckCircle2 className="h-2.5 w-2.5" /> : <Clock className="h-2.5 w-2.5" />}
                         {ipc.status}
                      </Badge>
                   </TableCell>
                   <TableCell className="pe-10 text-end">
                      <Button variant="ghost" size="icon" className="rounded-xl transition-all group-hover:bg-slate-900 group-hover:text-white">
