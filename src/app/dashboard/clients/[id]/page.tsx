...
export default function ClientDetailsPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20" dir={dir}>
      {/* Reduced Header Height and Font */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-4">
        <div className="flex items-center gap-3">
          <Button variant="ghost" onClick={() => router.push('/dashboard/clients')} className="h-9 w-9 p-0 rounded-xl bg-white shadow-sm border border-slate-200 hover:bg-slate-50">
            <ArrowRight className={cn("h-4 w-4", !isRtl && "rotate-180")} />
          </Button>
          <div className="text-start">
             <div className="flex items-center gap-3">
                <div className="px-3 h-8 rounded-lg bg-primary/5 flex items-center justify-center text-primary font-black text-xs border border-primary/10">
                   {client.fileNumber}
                </div>
        ...
        <div className="flex gap-2">
           <Button onClick={() => router.push(`/dashboard/clients/${clientId}/edit`)} variant="outline" size="sm" className="h-9 px-4 rounded-lg font-bold text-[10px] gap-2">
              <Edit3 className="h-3.5 w-3.5" /> {isRtl ? 'تعديل' : 'Edit'}
           </Button>
           <Button 
             onClick={() => router.push(`/dashboard/clients/${clientId}/transactions/new`)}
             size="sm"
             className="h-9 px-4 rounded-lg bg-primary text-white font-black text-[10px] gap-2 shadow-lg shadow-primary/10"
           >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
              ...
              <CardContent className="p-3 space-y-2">
                 {transactions?.map((trans) => (
                    <div 
                      ...
                      className="p-3 rounded-xl border border-slate-100 hover:border-primary/20 hover:bg-primary/5 transition-all cursor-pointer flex items-center justify-between group"
                    >
                       <div className="flex items-center gap-3">
                          <div className={cn(
                            "h-9 w-9 rounded-lg flex items-center justify-center shadow-sm transition-transform group-hover:scale-105",
                            trans.status === 'completed' ? "bg-emerald-50 text-emerald-600" : "bg-blue-50 text-blue-600"
                          )}>
                             <PlayCircle className="h-5 w-5" />
                          </div>
                          <div className="text-start">
                             <p className="text-[8px] font-black text-slate-400 uppercase tracking-tighter">{trans.transactionNumber}</p>
                             <h4 className="font-black text-[13px] text-slate-800 leading-tight">{trans.subServiceName}</h4>
                             <p className="text-[9px] font-bold text-primary mt-0.5">{trans.activityTypeName}</p>
                          </div>
                       </div>
                       <ArrowRight className={cn("h-4 w-4 text-slate-200 group-hover:text-primary transition-all", isRtl && "rotate-180")} />
                    </div>
                 ))}
                 {!transactions?.length && <div className="py-12 text-center text-[10px] text-slate-400 font-bold italic">لا يوجد معاملات مسجلة.</div>}
              </CardContent>
           </Card>
           
           {/* Visual Map Area - Reduced Height */}
           <Card className="border-0 shadow-lg rounded-2xl bg-white overflow-hidden ring-1 ring-black/5">
              <div className="grid grid-cols-1 md:grid-cols-2">
                 <div className="p-5 space-y-4 text-start border-e border-slate-50">
                    <div className="flex items-center gap-2 mb-2">
                       <Compass className="h-4 w-4 text-blue-600" />
                       <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500">{isRtl ? 'الرادار الجغرافي' : 'Location Radar'}</h3>
                    </div>
                    <div className="p-3 rounded-xl bg-slate-50 border border-white shadow-inner">
                       <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">{isRtl ? 'العنوان' : 'Address'}</p>
                       <p className="text-xs font-black text-slate-800">{client.governorateName} / {client.areaName}</p>
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                       {[{ l: 'B', v: client.block }, { l: 'S', v: client.street }, { l: 'P', v: client.houseNumber }].map((x, i) => (
                         <div key={i} className="p-2 rounded-lg bg-white border border-slate-100 text-center">
                            <span className="text-[8px] text-slate-400 block font-bold">{x.l}</span>
                            <span className="text-[11px] font-black text-slate-800">{x.v || '-'}</span>
                         </div>
                       ))}
                    </div>
                 </div>
                 ...
                 <div className="p-5 bg-slate-50/50 flex items-center justify-center">
                    <div className={cn(
                       "relative h-32 w-full rounded-[2rem] overflow-hidden shadow-lg border-2",
                       coordinates ? "bg-white border-blue-500/10 cursor-pointer hover:ring-4 hover:ring-blue-500/5 transition-all" : "bg-white/50 border-dashed border-slate-200"
                    )}>
                       {coordinates ? (
                          <div onClick={() => window.open(client.locationUrl, '_blank')} className="h-full w-full block">
                             ...
                             <div className="absolute bottom-2 left-0 right-0 px-2 pointer-events-none">
                                <Badge className="w-full bg-blue-600 text-white font-black text-[7px] uppercase h-4 border-0 justify-center">GPS LOCKED</Badge>
                             </div>
                          </div>
           ...
           <Card className="border-0 shadow-lg rounded-2xl bg-white p-5 text-start">
              <div className="flex items-center gap-2 mb-3">
                 <MessageSquare className="h-4 w-4 text-amber-600" />
                 <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500">{isRtl ? 'توثيق متابعة' : 'Log Visit'}</h3>
              </div>
              <Textarea 
                value={interaction} 
                onChange={e => setInteraction(e.target.value)}
                className="min-h-[70px] rounded-xl border-slate-200 text-xs p-3 focus:bg-slate-50 transition-all resize-none mb-3"
                placeholder={isRtl ? "اكتب تفاصيل الزيارة هنا..." : "Log details..."}
              />
              <div className="flex justify-end">
                 <Button onClick={handleLogVisit} disabled={logging || !interaction.trim()} size="sm" className="rounded-lg h-8 px-5 bg-slate-900 text-white font-black text-[10px]">
           ...
           <Card className="border-0 shadow-lg rounded-2xl bg-white overflow-hidden flex flex-col h-full min-h-[400px]">
              <CardHeader className="bg-slate-50/50 border-b p-4">
                 <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-[10px] font-black uppercase tracking-widest text-slate-500">{isRtl ? 'سجل العمليات' : 'History Log'}</CardTitle>
                 </div>
              </CardHeader>
              <CardContent className="p-0 flex-1 overflow-y-auto max-h-[500px] scrollbar-hide text-start">
                 <div className="relative p-5">
                    ...
                    <div className="space-y-5">
                          ...
                          <div key={event.id} className="relative ps-8">
                             ...
                             <div className="space-y-0.5">
                                <div className="flex justify-between items-center">
                                   <span className="text-[7px] font-black text-slate-400 uppercase tracking-tighter">{event.type}</span>
                                   <span className="text-[7px] font-mono text-slate-300 font-bold">{event.createdAt?.toDate().toLocaleDateString()}</span>
                                </div>
                       ...
                       {!history?.length && <div className="py-20 text-center text-[10px] text-slate-300 font-bold italic">لا يوجد أحداث تاريخية.</div>}
                    </div>
                 </div>
              </CardContent>
           </Card>
