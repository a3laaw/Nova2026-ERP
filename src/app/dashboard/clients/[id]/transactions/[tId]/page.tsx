                        ...
                        <CardContent className="p-0">
                           <div className="p-6 flex flex-col md:flex-row items-center justify-between gap-6">
                              ...
                              <div className="flex gap-2 shrink-0">
                                 ...
                                 <Button variant="ghost" size="icon" onClick={() => setOpenStages(prev => ({...prev, [stage.id!]: !prev[stage.id!]}))} className={cn("h-11 w-11 rounded-xl text-slate-300 hover:text-primary transition-all", isOpen && "bg-primary/5 text-primary")}><MessageSquare className="h-5 w-5" /></Button>
            ...
            <div className="p-8 space-y-6 text-start">
               {!activeBoq ? (
                 ...
                 <>
                    ...
                    <div className="space-y-2"><Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{isRtl ? "ملاحظات التنفيذ" : "Field Notes"}</Label><Textarea value={progressNotes} onChange={e => setProgressNotes(e.target.value)} className="min-h-[100px] rounded-2xl border-2 bg-slate-50/30 p-4 text-xs font-bold" /></div>
                 </>
               )}
            </div>
